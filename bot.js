...
if (!DISCORD_BOT_CHANNEL_ID) throw new Error('DISCORD_BOT_CHANNEL_ID environment variable is not set.');

const LISTEN_API_PORT = process.env.LISTEN_API_PORT;
if (!LISTEN_API_PORT) throw new Error('LISTEN_API_PORT environment variable is not set.');
...
if (!BOT_MANAGER_DISCORD_TOKEN) throw new Error('BOT_MANAGER_DISCORD_TOKEN environment variable is not set.');

const LISTEN_API_KEY = process.env.LISTEN_API_KEY;
if (!LISTEN_API_KEY) throw new Error('LISTEN_API_KEY environment variable is not set.');
...
app.use((req, res, next) => {
    ...
    if (apiKey !== LISTEN_API_KEY) {
        const clientIp = req.ip || req.headers['x-forwarded-for'] || req.connection.remoteAddress; // Get the client's IP
        console.log(`Invalid API key attempt from IP: ${clientIp}, Provided Key: ${apiKey || 'None'}`);
...
// Bot Manager Client
const botManager = new Client({ intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildMessages, GatewayIntentBits.MessageContent] });
...
function updateBotManagerPresence() {
    ...
    try {
        botManager.channels.fetch(DISCORD_BOT_CHANNEL_ID).then((channel) => {
            if (channel.isTextBased()) { // Ensure the channel is a text-based channel
                const topic = botCount > 0 ? `Active bots: ${botNames}` : 'No bots are currently active.';
            ...
            console.error(`Failed to fetch or update channel topic: ${err.message}`);
        });
    } catch (err) {
...
botManager.on('messageCreate', async (message) => {
    ...
    if (command === 'clear') {
        ...
        if (message.channel.id !== DISCORD_BOT_CHANNEL_ID) {
            return message.reply('This command can only be used in the designated bot channel.');
        }
        ...
        try {
            ...
            await message.channel.send('✅ All messages in this channel have been cleared.');
        } catch (error) {
            console.error(`Failed to clear messages in channel: ${error.message}`);
            await message.reply('❌ Failed to clear messages. Please try again later.');
        }
        ...
        if (!name || !token) {
            return message.reply('Please provide a bot name and token. Usage: !addbot <name> <token>');
        }
...
async function addBot(name, token) {
    ...
    bot.once('ready', async () => {
        ...
        try {
            ...
            if (channel && channel.isTextBased()) {
                await channel.send(`✅ Bot with name **${name}** successfully logged in as ${bot.user.tag}.`);
            }
        } catch (error) {
    ...
    bot.on('error', async (err) => {
        ...
        const now = new Date().toISOString().slice(0, 19).replace('T', ' ');
        await db.query('UPDATE bots SET enabled = 0, logonError = ?, disabledOn = ? WHERE name = ?', [err.message, now, name]);
        ...
        try {
            ...
            console.error(`Failed to send error message to channel: ${error.message}`);
        }
    });
    ...
    try {
        ...
        const now = new Date().toISOString().slice(0, 19).replace('T', ' ');
        await db.query('UPDATE bots SET enabled = 0, logonError = ?, disabledOn = ? WHERE name = ?', [error.message, now, name]);

        try {
            const channel = await botManager.channels.fetch(DISCORD_BOT_CHANNEL_ID);
            if (channel && channel.isTextBased()) {
                await channel.send(`❌ Failed to log in bot with name **${name}**: ${error.message}`);
            }
        } catch (err) {
            console.error(`Failed to send login failure message to channel: ${err.message}`);
        }
    }
}
...
app.post('/api/bot/presence', async (req, res) => {
    ...
    try {
        ...
        console.error(`Failed to update presence for bot ${name}: ${error.message}`);
